...
//
function addInferredSpreadProperties(
  spreadValue,
  spreadAstNode,
  elementType,
  scope
  ...
  if (scope !== null) {
    ...
    if (componentData !== undefined) {
      if (
        componentData.params !== undefined &&
        componentData.params.length > 0
      ) {
          ...
          if (!spreadValue.properties.has(key)) {
            if (value.accessedAsSpread === true) {
              spreadValue.properties.set(key, {
                descriptor: {
                  value: evaluator.createAbstractObjectOrUndefined(`this.props.${key}`),
                },
              });
            } else {
              spreadValue.properties.set(key, {
                descriptor: {
                  value: evaluator.createAbstractObject(`this.props.${key}`),
                },
              });
            }
          }
...
function evaluateJSXAttributes(
  ...
  if (defaultProps !== null) {
    const defaultPropsShape = convertAccessorsToNestedObject(
      null,
    ...
    for (let i = 0; i < defaultPropsAst.properties.length; i++) {
      ...
      attributes.set(
        name,
        GetValue(realm, env.evaluate(defaultPropAst.value, strictCode))
      );
    }
  }
  for (let astAttribute of astAttributes) {
    switch (astAttribute.type) {
      ...
      case 'JSXSpreadAttribute':
        ...
        const spreadValue = GetValue(
          realm,
          env.evaluate(astAttribute.argument, strictCode)
        );
        ...
        if (spreadValue.properties === undefined) {
          ...
          if (astAttributes.length > 1) {
            throw new Error(
              `An unknown JSXSpreadAttribute of "...${traverser.getNameFromAst(astAttribute.argument)}" with unknown properties was passed in.`
            );
          }
          spread = spreadValue;
        ...
        Object.keys(possibleProperties).forEach(key => {
          ...
          const val = GetValue(
            realm,
            env.evaluate(
              t.memberExpression(astAttribute.argument, t.identifier(key)),
              strictCode
            )
          );
          if (key === 'children') {
