...
function getFunctionReferenceName(functionValue) {
  return functionValue.__originalName || functionValue.properties.get('name').descriptor.value.value;
}
...
function convertReactElementToJSXExpression(objectValue) {
  ...
  let identifier = convertExpressionToJSXIdentifier(convertValueToExpression(typeValue));
  let attributes = [];
  let children = [];
...
function convertValueToExpression(value) {
  if (value instanceof AbstractValue) {
    let serializedArgs = value.args.map(abstractArg => convertValueToExpression(abstractArg));
  ...
  if (value instanceof ObjectValue) {
    if (value.properties.has('$$typeof')) { // TODO: Also compare the value to ensure it's the symbol
