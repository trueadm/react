...
import {
  ...
  initSyntheticEventDispatchQueue,
  createSyntheticEventDispatchQueueItem,
} from './DOMModernPluginEventSystem';

function accumulateTwoPhaseListenersForModernEventSystem(
  ...
  const {bubbled, captured} = initSyntheticEventDispatchQueue(event);
  ...
  if (
    ...
    const {
      bubbled: bubbledRegistrationName,
      captured: capturedRegistrationName,
    ...
    while (node !== null) {
      ...
      if (node.tag === HostComponent) {
        ...
        if (capturedRegistrationName !== null) {
          const callback = getListener(node, capturedRegistrationName);
          if (callback != null) {
            captured.push(
              createSyntheticEventDispatchQueueItem(callback, instance, node),
            );
          }
        }
        if (bubbledRegistrationName !== null) {
          const callback = getListener(node, bubbledRegistrationName);
          if (callback != null) {
            bubbled.push(
              createSyntheticEventDispatchQueueItem(callback, instance, node),
            );
        ...
        if (eventTypeMap !== undefined) {
          ...
          if (listeners !== undefined) {
            const captureListeners = Array.from(listeners.captured);
            const bubbleListeners = Array.from(listeners.bubbled);
            let lastHostElement = ((lastHostComponent: any): Element);
...
function accumulateTwoPhaseListenersForLegacyEventSystem(
  event: ReactSyntheticEvent,
): void {
  const phasedRegistrationNames = event.dispatchConfig.phasedRegistrationNames;
...
export default function accumulateTwoPhaseListeners(
  ...
  if (enableModernEventSystem) {
    accumulateTwoPhaseListenersForModernEventSystem(
      event,
    ...
    accumulateTwoPhaseListenersForLegacyEventSystem(event);
  }
}
